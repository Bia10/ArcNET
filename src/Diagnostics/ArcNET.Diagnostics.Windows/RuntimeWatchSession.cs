...
public sealed class RuntimeWatchSession : IDisposable
{
    ...
    public static RuntimeWatchSession Install(ProcessMemory memory, IReadOnlyList<RuntimeWatchHookDefinition> hooks)
    {
        ...
        try
        {
            ...
            return new RuntimeWatchSession(
                buffer,
                [.. installedHooks],
                definitionsById,
    ...
    public RuntimeWatchReadResult ReadSince(uint lastSequence)
    {
        ...
        if (writeSequence <= lastSequence)
            return new RuntimeWatchReadResult(writeSequence, 0, 0, contentionDrops, []);
    ...
    private sealed class RemoteEventBuffer : IDisposable
    {
        ...
        public const int RecordSize = sizeof(uint) * (3 + StackCaptureDwordCount);
        ...
        public readonly record struct BufferSnapshot(byte[] Bytes, uint WriteSequence, uint ContentionDropCount)
        {
            public RuntimeWatchRawRecord ReadRecord(uint sequence)
            {
                ...
                var returnAddress = BinaryPrimitives.ReadUInt32LittleEndian(span.Slice(sizeof(uint) * 2, sizeof(uint)));
                RuntimeWatchStackCapture stackDwords = default;
                for (var index = 0; index < StackCaptureDwordCount; index++)
                {
                    stackDwords.Set(
                        index,
                        BinaryPrimitives.ReadUInt32LittleEndian(span.Slice(sizeof(uint) * (index + 3), sizeof(uint)))
                    );
                }

                return new RuntimeWatchRawRecord(recordSequence, hookId, returnAddress, stackDwords);
            }
        }
    }
        ...
        )
        {
            return AssembleCode(
                ...
                assembler =>
                {
                    ...
                    var epilogueLabel = assembler.CreateLabel("epilogue");
                    ...
                    // Publish the per-slot sequence only after the payload is fully written.
