...
{
    public static InterceptService Default { get; } = new(new InterceptBackend());
    ...
    private static void Validate(InterceptStartRequest request)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Interception stack capture dword count must be between 1 and {RuntimeInterceptionSession.MaximumStackCaptureDwordCount}."
            );
        }

        foreach (var argumentOverride in request.Mutation.ArgumentOverrides)
        {
            if (argumentOverride.Index is < 0 or >= RuntimeInterceptionSession.MaximumStackCaptureDwordCount)
            {
                throw new InvalidOperationException(
                    $"Argument override index must be between 0 and {RuntimeInterceptionSession.MaximumStackCaptureDwordCount - 1}."
                );
            }
        }
        ...
        foreach (var dereference in request.Dereferences)
        {
            ...
            if (
                ...
                && dereference.Index is < 0 or >= RuntimeInterceptionSession.MaximumStackCaptureDwordCount
            )
            {
                throw new InvalidOperationException(
                    $"Stack dereference index must be between 0 and {RuntimeInterceptionSession.MaximumStackCaptureDwordCount - 1}."
                );
            }
        }
    ...
    private static RuntimeInterceptionMutation CreateMutation(InterceptStartRequest request)
    {
        ...
        var argumentOverrides = new uint[RuntimeInterceptionSession.MaximumStackCaptureDwordCount];
