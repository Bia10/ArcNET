...
public sealed class ReadService(IReadBackend backend)
{
    public ReadSnapshot Read(ReadRequest request)
    {
        ...
        try
        {
            return Normalize(request.AdapterKey) switch
            {
                ...
                "fieldlength" => ReadFieldLength(request),
                ...
                "questglobal" => ReadQuestGlobal(request),
    ...
    private ReadSnapshot ReadQuest(ReadRequest request)
    {
        ...
        return CreateSuccessSnapshot(
            ...
            [
                ...
                Value("quest_state", "Quest State", RuntimeWatchValueCatalog.QuestPcStateName(nativeRead.Int32Value)),
                Value("raw_state", "Raw State", nativeRead.Int32Value),
            ]
        );
    }

    private ReadSnapshot ReadQuestGlobal(ReadRequest request)
    {
        RequireArgumentCount(request.Arguments, 1, 1, "quest-global <quest-id>");
        var questId = ParseInt32(request.Arguments[0]);
        var nativeRead = InvokeInt32(request, "quest_global_state_get", [unchecked((uint)questId)]);

        return CreateSuccessSnapshot(
            request,
            $"Read global quest state for quest {questId}.",
            target: null,
            nativeRead,
            [
                Value("quest_id", "Quest Id", questId),
                Value(
                    "quest_state",
                    "Quest State",
                    RuntimeWatchValueCatalog.QuestStateName(
                        RuntimeWatchValueCatalog.QuestBaseState(nativeRead.Int32Value)
                    )
                ),
                Value("raw_state", "Raw State", nativeRead.Int32Value),
            ]
    ...
    private ReadSnapshot ReadFieldLength(ReadRequest request)
    {
        RequireArgumentCount(request.Arguments, 2, 2, "field-length <object-handle|player|auto> <field-id|raw-name>");
        var target = ResolveTarget(request, request.Arguments[0], "field-length target");
        ...
        var length = Math.Max(0, nativeRead.Int32Value);

        return CreateSuccessSnapshot(
            request,
            $"Read array length for {ObjectFieldCatalog.CollectionName(fieldId)} on {target.TargetText}.",
            target,
            nativeRead,
            [
