...
public sealed class ObjectProbeService(IObjectProbeBackend backend)
{
    ...
    private static IReadOnlyList<ObjectProbeSectionSnapshot> CreateSections(IReadOnlyList<LiveObjectDetail> details)
    {
        ...
        AddSection(
            ...
            "Inventory Links",
            static detail =>
                detail.Key.StartsWith("obj_f_critter_inventory_list_idx_", StringComparison.OrdinalIgnoreCase)
                || detail.Key.StartsWith("obj_f_container_inventory_list_idx_", StringComparison.OrdinalIgnoreCase)
        );
        AddSection(
            sections,
            remaining,
    ...
    private static string DescribeResolutionSource(string resolutionSource) =>
        resolutionSource switch
        {
            ...
            "obj_field_handle_get" => "Getter-backed handle read",
            "obj_array_field_int32_get" => "Getter-backed array read",
            "obj_array_field_handle_get" => "Getter-backed handle-array read",
