...
public sealed class EditorWorkspaceSessionTests
{
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_CreateTrackedMapWorldEditShell_ToleratesTerrainPaletteThatDoesNotFitInMemory()
    {
        const ulong sectorKey = 101334386389UL;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        ...
        try
        {
            ...
            MapPropertiesFormat.WriteToFile(in mapProperties, Path.Combine(contentDir, "maps", "map01", "map.prp"));
            SectorFormat.WriteToFile(
                new SectorBuilder(MakeSector()).SetTile(5, 6, 201u).Build(),
                Path.Combine(contentDir, "maps", "map01", $"{sectorKey}.sec")
            );
            ...
            await Assert.That(shell.TerrainPalette.SelectedEntry).IsNull();
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    [Test]
    public async Task SectorObjectHelpers_WallPalettePlacement_SynthesizesCurrentAidForGhostAndCommit()
    {
        const int protoNumber = 1021;
        var expectedWallArtId = new ArtId(0x10100000u);
        const ulong sectorKey = 101334386389UL;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        ...
        try
        {
            ...
            MessageFormat.WriteToFile(
                new MesFile { Entries = [new MessageEntry(1, "TOU1 TOU1 nul nul")] },
                Path.Combine(contentDir, "art", "wall", "structure.mes")
            );
            ArtFormat.WriteToFile(
                CreateArtFile(width: 2, height: 2, pixels: [1, 1, 1, 1]),
                Path.Combine(contentDir, "art", "wall", "TOUbseU0.art")
            );
            SectorFormat.WriteToFile(
                new SectorBuilder(MakeSector()).SetTile(6, 6, 200u).Build(),
                Path.Combine(contentDir, "maps", "map01", $"{sectorKey}.sec")
            );
            ...
            await Assert.That(paletteEntry).IsNotNull();
            await Assert.That(paletteEntry!.CurrentArtId).IsEqualTo(expectedWallArtId);
            ...
            await Assert.That(preview.Objects.Count).IsEqualTo(1);
            await Assert.That(preview.Objects[0].CurrentArtId).IsEqualTo(expectedWallArtId);
            ...
            var updatedPreview = updatedWorkspace.CreateMapScenePreview(
                "map01",
                EditorArtResolverBindingStrategy.ArcanumMessageTables
            );
            var placedObject = updatedPreview.Sectors.Single().Objects.Single();

            await Assert.That(placedObject.CurrentArtId).IsEqualTo(expectedWallArtId);
            await Assert.That(placedObject.SpriteBounds).IsNotNull();
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    public async Task MapBootstrapHelpers_CreateDefaultMapWorldEditScene_AutoBindsConservativeNumericArtForPaintableScene()
    {
        ...
        try
        {
            ...
            await Assert.That(scene.PaintableScene.Items.Any(item => item.SpriteReference is not null)).IsTrue();
    ...
    public async Task MapBootstrapHelpers_CreateMapWorldEditScene_ComposesViewportPaintableSceneAndPlacementGhost()
    {
        ...
        try
        {
            ...
            var sceneRequest = new EditorMapWorldEditSceneRequest
            {
                RenderRequest = new EditorMapFloorRenderRequest
                {
                    ViewMode = EditorMapSceneViewMode.Isometric,
            ...
            await Assert
                .That(
                    worldScene.PaintableScene.Items.Any(item =>
                        item.Kind == EditorMapRenderQueueItemKind.FloorTile && item.SpriteReference is not null
                    )
                )
                .IsTrue();
            await Assert
                .That(
                    worldScene.PaintableScene.Items.Any(item =>
                        item.Kind == EditorMapRenderQueueItemKind.PlacementPreviewObject
                        && item.SpriteReference is not null
                    )
                )
                .IsTrue();
            ...
            await Assert.That(asyncWorldScene.SceneRender.Tiles.Count).IsEqualTo(worldScene.SceneRender.Tiles.Count);
            await Assert
                .That(asyncWorldScene.PaintableScene.Items.Count)
                .IsEqualTo(worldScene.PaintableScene.Items.Count);
            await Assert
                .That(asyncWorldScene.SpriteCoverage.ResolvedArtIds.Count)
                .IsEqualTo(worldScene.SpriteCoverage.ResolvedArtIds.Count);
            await Assert
                .That(asyncWorldScene.ViewportLayout.ViewportWidth)
                .IsEqualTo(worldScene.ViewportLayout.ViewportWidth);
            await Assert
                .That(asyncWorldScene.ViewportLayout.ViewportHeight)
                .IsEqualTo(worldScene.ViewportLayout.ViewportHeight);
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_ApplyTrackedObjectPlacementTool_UsesProvidedTileSelection()
    {
        const int protoNumber = 1001;
        const ulong sectorKey = 101334386389UL;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        ...
        try
        {
            ...
            _ = session.SetTrackedObjectPlacementEntry("map-view-1", protoNumber);

            var createdObjects = session.ApplyTrackedObjectPlacementTool(
                "map-view-1",
                new EditorProjectMapSelectionState { SectorAssetPath = sectorAssetPath, Tile = new Location(11, 12) }
            );
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_GetTrackedObjectPaletteSummary_WithoutSearch_DoesNotMaterializeFullPalette()
    {
        const int guardProtoNumber = 1001;
        const int wolfProtoNumber = 1002;
        ...
        try
        {
            ...
            var emptySummary = session.GetTrackedObjectPaletteSummary("map-view-1");
            var fullSummary = session.GetTrackedObjectPaletteSummary(
                "map-view-1",
                includeFullPaletteWhenSearchIsEmpty: true
            );

            _ = session.SelectTrackedObjectPaletteEntry("map-view-1", wolfProtoNumber);
            var selectedSummary = session.GetTrackedObjectPaletteSummary("map-view-1");

            await Assert.That(emptySummary.CanBrowse).IsFalse();
            await Assert.That(emptySummary.Entries.Count).IsEqualTo(0);
            await Assert.That(emptySummary.AvailableCategories.Count).IsEqualTo(0);
            await Assert.That(fullSummary.CanBrowse).IsTrue();
            await Assert.That(fullSummary.Entries.Count).IsEqualTo(2);
            await Assert
                .That(fullSummary.Entries.Select(static entry => entry.ProtoNumber).ToArray())
                .IsEquivalentTo([guardProtoNumber, wolfProtoNumber]);
            await Assert.That(selectedSummary.SearchText).IsNull();
            await Assert.That(selectedSummary.CanBrowse).IsTrue();
            await Assert.That(selectedSummary.Entries.Count).IsEqualTo(1);
            await Assert.That(selectedSummary.Entries[0].ProtoNumber).IsEqualTo(wolfProtoNumber);
            await Assert.That(selectedSummary.SelectedEntry).IsNotNull();
            await Assert.That(selectedSummary.SelectedEntry!.ProtoNumber).IsEqualTo(wolfProtoNumber);
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_GetTrackedObjectSelectionSummary_WithSceneRender_UsesSuppliedRenderObjects()
    {
        const int protoNumber = 1001;
        const ulong sectorKey = 101334386389UL;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        ...
        try
        {
            ProtoFormat.WriteToFile(MakeProto(protoNumber), Path.Combine(contentDir, "proto", "001001 - Hidden.pro"));
            ...
            var hiddenObjectsRender = session.CreateMapFloorRenderPreview(
                "map-view-1",
                new EditorMapFloorRenderRequest
                {
                    ViewMode = EditorMapSceneViewMode.TopDown,
                    ...
                    IncludeLightOverlays = false,
                    IncludeScriptOverlays = false,
                }
            );

            var summaryFromWorkspace = session.GetTrackedObjectSelectionSummary("map-view-1");
            var summaryFromRender = session.GetTrackedObjectSelectionSummary("map-view-1", hiddenObjectsRender);

            await Assert.That(summaryFromWorkspace.SelectedObjects.Count).IsEqualTo(1);
            await Assert.That(summaryFromRender.SelectedObjects).IsEmpty();
            await Assert.That(summaryFromRender.MissingObjectIds).IsEquivalentTo([selectedObject.Header.ObjectId]);
            await Assert.That(summaryFromRender.SectorAssetPaths).IsEmpty();
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_GetTrackedObjectSelectionSummary_WithSceneRender_PreservesEditableObjectMetadata()
    {
        const int protoNumber = 1001;
        const ulong sectorKey = 101334386389UL;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        ...
        try
        {
            ProtoFormat.WriteToFile(
                MakeProto(protoNumber),
                Path.Combine(contentDir, "proto", "001001 - EditableSelection.pro")
            );

            var selectedObject = new MobDataBuilder(MakePc(protoNumber))
                .WithLocation(5, 6)
                .WithOffset(12, -8)
                .WithRotation(0.75f)
                .WithRotationPitch(1.25f)
                .Build();
            SectorFormat.WriteToFile(
                MakeSector(selectedObject),
                Path.Combine(contentDir, "maps", "map01", $"{sectorKey}.sec")
            ...
            var renderItem = new EditorMapObjectRenderItem
            {
                SectorAssetPath = sectorAssetPath,
                ObjectId = selectedObject.Header.ObjectId,
                ProtoId = selectedObject.Header.ProtoId,
                ObjectType = selectedObject.Header.GameObjectType,
                CurrentArtId = new ArtId((uint)(selectedObject.GetProperty(ObjectField.CurrentAid)?.GetInt32() ?? 0)),
                MapTileX = 5,
                MapTileY = 6,
                Tile = new Location(5, 6),
                DrawOrder = 0,
            ...
            await Assert.That(selectionSummary.SelectedObjects.Count).IsEqualTo(1);
            await Assert.That(selectionSummary.SelectedObjects[0].OffsetX).IsEqualTo(12);
            await Assert.That(selectionSummary.SelectedObjects[0].OffsetY).IsEqualTo(-8);
            await Assert.That(selectionSummary.SelectedObjects[0].Rotation).IsEqualTo(0.75f);
            await Assert.That(selectionSummary.SelectedObjects[0].RotationPitch).IsEqualTo(1.25f);
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_GetTrackedObjectSelectionSummary_WithSceneRender_ResolvesNullIdObjectFromUniqueTile()
    {
        const int protoNumber = 1001;
        const ulong sectorKey = 101334386389UL;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        ...
        try
        {
            ProtoFormat.WriteToFile(
                MakeProto(protoNumber),
                Path.Combine(contentDir, "proto", "001001 - NullIdSelection.pro")
            );

            var retainedObject = new MobDataBuilder(ObjectType.Pc, nullObjectId, MakeProtoId(protoNumber))
                .WithLocation(8, 9)
                .Build();
            var selectedObject = new MobDataBuilder(ObjectType.Pc, nullObjectId, MakeProtoId(protoNumber))
                .WithLocation(5, 6)
                .WithOffset(12, -8)
                .WithRotation(0.75f)
                .WithRotationPitch(1.25f)
                .Build();
            SectorFormat.WriteToFile(
                MakeSector(retainedObject, selectedObject),
                Path.Combine(contentDir, "maps", "map01", $"{sectorKey}.sec")
            );
            ...
            await Assert.That(selectionSummary.SelectedObjects.Count).IsEqualTo(1);
            await Assert.That(selectionSummary.SelectedObjects[0].Location).IsEqualTo(new Location(5, 6));
            await Assert.That(selectionSummary.SelectedObjects[0].OffsetX).IsEqualTo(12);
            await Assert.That(selectionSummary.SelectedObjects[0].OffsetY).IsEqualTo(-8);
            await Assert.That(selectionSummary.SelectedObjects[0].Rotation).IsEqualTo(0.75f);
            await Assert.That(selectionSummary.SelectedObjects[0].RotationPitch).IsEqualTo(1.25f);
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_GetTrackedObjectSelectionSummary_WithSceneRender_DoesNotOverflowLargeObjectLocations()
    {
        const int protoNumber = 1001;
        const ulong sectorKey = 101334386389UL;
        const int worldTileX = 84992;
        const int worldTileY = 44910;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        ...
        try
        {
            ProtoFormat.WriteToFile(
                MakeProto(protoNumber),
                Path.Combine(contentDir, "proto", "001001 - WorldSelection.pro")
            );
            ...
            var workspace = await EditorWorkspaceLoader.LoadAsync(contentDir);
            var session = workspace.CreateSession();
            var sectorLocalTile = new Location(worldTileX & 0x3F, worldTileY & 0x3F);
            _ = session.SetMapViewState(
                new EditorProjectMapViewState
                {
                    Id = "map-view-1",
            ...
            await Assert.That(selectionSummary.SelectedObjects.Count).IsEqualTo(1);
            await Assert.That(selectionSummary.SelectedObjects[0].ObjectId).IsEqualTo(selectedObject.Header.ObjectId);
            await Assert.That(selectionSummary.SelectedObjects[0].Location).IsEqualTo(sectorLocalTile);
            await Assert.That(selectionSummary.SelectedObjects[0].SourceAssetPath).IsEqualTo(sectorAssetPath);
        }
        finally
        {
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_GetTrackedObjectInspectorFlagsSummary_ResolvesNullIdWallBySelectedTile()
    {
        const int protoNumber = 1021;
        const ulong sectorKey = 101334386389UL;
        const int selectedWallFlags = 111;
        const int otherWallFlags = 222;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        var nullObjectId = new GameObjectGuid(GameObjectGuid.OidTypeNull, 0, 0, Guid.Empty);
        ...
        {
            ProtoFormat.WriteToFile(
                MakeWallProto(protoNumber),
                Path.Combine(contentDir, "proto", "001021 - InspectorWall.pro")
            );
            ...
            SectorFormat.WriteToFile(
                MakeSector(selectedWall, otherWall),
                Path.Combine(contentDir, "maps", "map01", $"{sectorKey}.sec")
            );
            ...
            var inspector = session.GetTrackedObjectInspectorSummary("map-view-1");
            var flagsSummary = session.GetTrackedObjectInspectorFlagsSummary("map-view-1");

            await Assert.That(inspector.TargetKind).IsEqualTo(EditorObjectInspectorTargetKind.SelectedObject);
            await Assert.That(inspector.SelectedObject).IsNotNull();
            await Assert.That(inspector.SelectedObject!.Location).IsEqualTo(new Location(5, 6));
            await Assert.That(flagsSummary.WallFlags).IsEqualTo(selectedWallFlags);
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_GetTrackedObjectInspectorFlagsSummary_ResolvesNullIdWallBySelectedPreviewMetadata()
    {
        const int protoNumber = 1021;
        const ulong sectorKey = 101334386389UL;
        const int selectedWallFlags = 111;
        ...
        try
        {
            ...
            var selectionSummary = new EditorMapObjectSelectionSummary
            {
                ...
                [
                    new EditorMapObjectPreview
                    {
                        ...
                        WallFlags = selectedWallFlags,
                        RotationPitch = 0f,
                    },
                ],
                MissingObjectIds = [],
                SectorAssetPaths = [sectorAssetPath],
            };

            var inspector = session.GetTrackedObjectInspectorSummary("map-view-1", selectionSummary);
            var flagsSummary = session.GetTrackedObjectInspectorFlagsSummary(inspector);
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_GetTrackedObjectInspectorScriptAttachmentsSummary_UsesSelectedSourceObjectIndex()
    {
        const int protoNumber = 1001;
        const ulong sectorKey = 101334386389UL;
        const int otherScriptId = 111;
        const int selectedScriptId = 222;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        var nullObjectId = new GameObjectGuid(GameObjectGuid.OidTypeNull, 0, 0, Guid.Empty);
        ...
        try
        {
            ...
            var otherObject = WithProperties(
                new MobDataBuilder(ObjectType.Npc, nullObjectId, MakeProtoId(protoNumber)).WithLocation(5, 6).Build(),
                MakeScriptProperty(otherScriptId)
            );
            var selectedObject = WithProperties(
                new MobDataBuilder(ObjectType.Npc, nullObjectId, MakeProtoId(protoNumber)).WithLocation(5, 6).Build(),
                MakeScriptProperty(selectedScriptId)
            );

            SectorFormat.WriteToFile(
                MakeSector(otherObject, selectedObject),
                Path.Combine(contentDir, "maps", "map01", $"{sectorKey}.sec")
            );
            ...
            var inspector = session.GetTrackedObjectInspectorSummary("map-view-1", selectionSummary);
            var attachments = session.GetTrackedObjectInspectorScriptAttachmentsSummary(inspector);

            await Assert.That(inspector.TargetKind).IsEqualTo(EditorObjectInspectorTargetKind.SelectedObject);
            await Assert.That(attachments.Attachments[0].ScriptId).IsEqualTo(selectedScriptId);
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    public async Task MapViewWorldEditToolHelpers_CreateTrackedMapWorldEditShell_BundlesTopDownSceneAndWorkflowSummaries()
    {
        ...
        try
        {
            ...
            var shellRequest = new EditorMapWorldEditShellRequest
            {
                ViewMode = EditorMapSceneViewMode.TopDown,
                ViewportWidth = 320d,
                ViewportHeight = 200d,
            ...
            var asyncShell = await session.CreateTrackedMapWorldEditShellAsync(
                "map-view-1",
                shellRequest,
                progress: new CallbackProgress<EditorMapWorldEditComposeProgress>(update =>
                {
                    lock (progressGate)
                    {
                        composeProgressUpdates.Add(update);
                    }
                })
            );
            EditorMapWorldEditComposeProgress[] progressUpdates;
            lock (progressGate)
            {
                progressUpdates = [.. composeProgressUpdates];
            }
            ...
            await Assert.That(asyncShell.MapName).IsEqualTo(shell.MapName);
            await Assert.That(asyncShell.Scene.SceneRender.ViewMode).IsEqualTo(shell.Scene.SceneRender.ViewMode);
            await Assert
                .That(asyncShell.Scene.ViewportLayout.ViewportWidth)
                .IsEqualTo(shell.Scene.ViewportLayout.ViewportWidth);
            await Assert
                .That(asyncShell.Scene.ViewportLayout.ViewportHeight)
                .IsEqualTo(shell.Scene.ViewportLayout.ViewportHeight);
            await Assert.That(asyncShell.ObjectPalette.Entries.Count).IsEqualTo(shell.ObjectPalette.Entries.Count);
            await Assert
                .That(asyncShell.ObjectSelection.SelectedObjects.Count)
            ...
            await Assert.That(progressUpdates.Any(static update => update.DominantElapsed is not null)).IsTrue();
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_CreateTrackedMapWorldEditShell_SkipsTrackedPlacementPreviewOutsidePlacementMode()
    {
        const int protoNumber = 1001;
        const ulong sectorKey = 101334386389UL;
        ...
        try
        {
            ...
            _ = session.SetMapViewState(
                new EditorProjectMapViewState
                {
                    ...
                    WorldEdit = new EditorProjectMapWorldEditState
                    {
                        ActiveTool = EditorProjectMapWorldEditActiveTool.None,
                    },
                }
            );
            ...
            await Assert.That(shell.TrackedPlacementPaintableScene).IsNull();
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_CreateTrackedMapWorldEditShell_BuildsOverlayOnlyPlacementPaintableScene()
    {
        const int protoNumber = 1001;
        const ulong sectorKey = 101334386389UL;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        ...
        try
        {
            ...
            await Assert.That(shell.HasTrackedPlacementPreview).IsTrue();
            await Assert.That(shell.TrackedPlacementPreview).IsNotNull();
            await Assert.That(shell.TrackedPlacementPaintableScene).IsNotNull();
            await Assert
                .That(shell.TrackedPlacementPreview!.RenderQueue.Count)
                .IsGreaterThan(shell.TrackedPlacementPreview.Objects.Count);
            await Assert
                .That(
                    shell.TrackedPlacementPaintableScene!.Items.All(static item =>
                        item.Kind is EditorMapRenderQueueItemKind.PlacementPreviewObject
                    )
                )
                .IsTrue();
            await Assert
                .That(shell.TrackedPlacementPaintableScene.Items.Count)
                .IsEqualTo(shell.TrackedPlacementPreview.Objects.Count);
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_PreviewTrackedObjectPlacementToolOverlay_UsesExistingSceneRenderQueue()
    {
        const int protoNumber = 1001;
        const ulong sectorKey = 101334386389UL;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        ...
        try
        {
            ...
            var preview = session.PreviewTrackedObjectPlacementToolOverlay(
                "map-view-1",
                selection,
                shell.Scene.SceneRender,
                shell.RenderRequest
            );

            await Assert.That(shell.TrackedPlacementPreview).IsNull();
            await Assert.That(preview.Objects.Count).IsEqualTo(1);
            await Assert.That(preview.RenderQueue.Count).IsEqualTo(preview.Objects.Count);
            await Assert
                .That(
                    preview.RenderQueue.All(static item =>
                        item.Kind is EditorMapRenderQueueItemKind.PlacementPreviewObject
                    )
                )
                .IsTrue();
            await Assert.That(preview.RenderQueue.Count).IsLessThan(shell.Scene.SceneRender.RenderQueue.Count);
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    public async Task MapViewWorldEditToolHelpers_CreateTrackedMapWorldEditShell_UsesCustomSpriteSource()
    {
        ...
        try
        {
            ...
            _ = shell.Scene.PaintableScene.Items.Any(item => item.SpriteReference is { ArtId.Value: not 0u });

            await Assert.That(spriteSource.ResolvedArtIds).IsEmpty();
            await Assert.That(spriteSource.MetricCheckedArtIds.Count).IsGreaterThan(0);
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_CreateTrackedMapWorldEditShell_UsesDefaultLazyRenderableResolverForSprites()
    {
        const ulong sectorKey = 101334386389UL;
        ...
        try
        {
            ...
            await Assert.That(asyncShell.Scene.SpriteCoverage.ResolvedSpriteReferenceCount).IsGreaterThan(0);
            await Assert
                .That(
            ...
            await Assert
                .That(
                    asyncShell.Scene.PaintableScene.Items.Any(item =>
                        item.Kind == EditorMapRenderQueueItemKind.FloorTile && item.SpriteReference is not null
                    )
                )
                .IsTrue();
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_CreateTrackedMapWorldEditShell_IncludesEmptyTileSurfaceForEditing()
    {
        const ulong sectorKey = 101334386389UL;
        ...
        try
        {
            ...
            var hit = EditorMapSceneRenderSpaceMath.HitTestScene(
                shell.Scene.SceneRender,
                shell.Scene.ViewportLayout,
                64d,
                64d
            );

            await Assert.That(shell.RenderRequest.IncludeEmptyTiles).IsTrue();
            await Assert.That(shell.Scene.SceneRender.Tiles.Count).IsGreaterThan(0);
            await Assert.That(shell.Scene.SceneRender.RenderQueue.Count).IsGreaterThan(0);
            await Assert
                .That(shell.Scene.PaintableScene.Items.Any(item => item.Kind == EditorMapRenderQueueItemKind.FloorTile))
                .IsTrue();
            await Assert.That(hit).IsNotNull();
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_CreateDefaultMapViewState_CentersSparseMapsOnOccupiedContent()
    {
        const ulong southWestSectorKey = 101334386389UL;
        const ulong southEastSectorKey = southWestSectorKey + 200UL;
        const ulong northWestSectorKey = southWestSectorKey + (200UL << 26);
        const ulong northEastSectorKey = northWestSectorKey + 200UL;
        ...
        try
        {
            ...
            SectorFormat.WriteToFile(
                MakeSector(),
                Path.Combine(contentDir, "maps", "map01", $"{northEastSectorKey}.sec")
            );
            ...
            var centerHit = EditorMapSceneRenderSpaceMath.HitTestScene(
                shell.Scene.SceneRender,
                shell.Scene.ViewportLayout,
                160d,
                100d
            ...
            await Assert.That(centerHit).IsNotNull();
            await Assert
                .That(shell.Scene.PaintableScene.Items.Any(item => item.Kind == EditorMapRenderQueueItemKind.FloorTile))
                .IsTrue();
        }
    ...
    public async Task MapViewWorldEditToolHelpers_FocusMapTarget_OpensLooseMapMobAndCentersResolvedPlacement()
    {
        ...
        try
        {
            ...
            MapProperties map01Properties = new()
            {
                ArtId = 200,
                Unused = 0,
                LimitX = 1,
            ...
            MapPropertiesFormat.WriteToFile(in map01Properties, Path.Combine(contentDir, "maps", "map01", "map.prp"));

            MapProperties map02Properties = new()
            {
                ArtId = 201,
                Unused = 0,
                LimitX = 1,
            ...
            MapPropertiesFormat.WriteToFile(in map02Properties, Path.Combine(contentDir, "maps", "map02", "map.prp"));

            SectorFormat.WriteToFile(
                new SectorBuilder(MakeSector()).SetTile(5, 5, 200u).Build(),
                Path.Combine(contentDir, "maps", "map01", $"{map01SectorKey}.sec")
            );
            SectorFormat.WriteToFile(
                new SectorBuilder(MakeSector()).SetTile(18, 45, 201u).Build(),
                Path.Combine(contentDir, "maps", "map02", $"{map02SectorKey}.sec")
            );
            ...
            var target = session.FocusMapTarget("map-view-1", mapMobAssetPath);
            var updatedMapViewState = session.GetMapViewStates().Single(state => state.Id == "map-view-1");

            await Assert.That(target.MapName).IsEqualTo("map02");
            await Assert.That(target.SectorAssetPath).IsEqualTo(map02SectorAssetPath);
            await Assert.That(target.Tile).IsEqualTo(new Location(18, 45));
            await Assert.That(target.CenterTileX).IsEqualTo(18d);
            await Assert.That(target.CenterTileY).IsEqualTo(45d);
            await Assert.That(target.ObjectId).IsEqualTo(placedMobId);
            await Assert.That(target.FocusAssetPath).IsEqualTo(mapMobAssetPath);
            await Assert.That(target.SourceAssetPath).IsEqualTo(mapMobAssetPath);
            await Assert.That(target.MatchCount).IsEqualTo(1);
            await Assert.That(updatedMapViewState.MapName).IsEqualTo("map02");
            await Assert.That(updatedMapViewState.Camera.CenterTileX).IsEqualTo(18d);
            await Assert.That(updatedMapViewState.Camera.CenterTileY).IsEqualTo(45d);
            await Assert.That(updatedMapViewState.Camera.Zoom).IsEqualTo(1.25d);
            await Assert.That(updatedMapViewState.Selection.SectorAssetPath).IsEqualTo(map02SectorAssetPath);
            await Assert.That(updatedMapViewState.Selection.Tile).IsEqualTo(new Location(18, 45));
            await Assert.That(updatedMapViewState.Selection.ObjectId).IsEqualTo(placedMobId);
            await Assert.That(session.ActiveAssetPath).IsEqualTo(mapMobAssetPath);
            await Assert.That(session.GetOpenAssets().Any(asset => asset.AssetPath == mapMobAssetPath)).IsTrue();
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    [Test]
    public async Task MapViewWorldEditToolHelpers_CreateTrackedMapWorldEditShell_ToleratesMalformedCritterProgressionArrays()
    {
        const int protoNumber = 1001;
        const ulong sectorKey = 101334386389UL;
        var sectorAssetPath = $"maps/map01/{sectorKey}.sec";
        ...
        try
        {
            ...
            await Assert
                .That(shell.ObjectInspector.TargetKind)
                .IsEqualTo(EditorObjectInspectorTargetKind.SelectedObject);
            await Assert.That(shell.ObjectInspectorCritterProgression.IsCritterTarget).IsTrue();
            await Assert.That(shell.ObjectInspectorCritterProgression.Level).IsEqualTo(0);
            await Assert.That(shell.ObjectInspectorCritterProgression.SkillBow).IsEqualTo(0);
        }
        finally
        {
            if (Directory.Exists(contentDir))
    ...
    private sealed class TestSpriteSource : IEditorMapRenderSpriteSource
    {
        ...
        public bool CanResolve(ArtId artId, EditorMapRenderSpriteRequest? request = null)
        {
            ...
            CoverageCheckedArtIds.Add(artId);
            return true;
        }
