...
/// When mobile objects are updated their bytes are recomputed, the TFAI index is rebuilt
...
public static class SaveGameWriter
{
    ...
    /// Pass <see langword="null"/> to leave all mobiles unchanged.
    /// </param>
    ...
    {
        // 1. Build updated files dictionary.
    ...
    /// </summary>
    /// <inheritdoc cref="Save(SaveGame,string,string,string,SaveInfo?,IReadOnlyDictionary{string,MobData}?)"/>
    public static void Save(
        SaveGame original,
        string saveFolder,
        string slotName,
