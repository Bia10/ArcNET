...
internal static class RenderBufferDumpCommand
{
    ...
    {
        Console.WriteLine("RenderBufferDump - dump render queue and paintable scene items for one map");
        Console.WriteLine();
        Console.WriteLine(
            "Usage: RenderBufferDump <game-root-dir> [--map <mapName>] [--module <moduleName>] [--kind <kind>] [--layer <layer>] [--asset-contains <text>] [--art-id <hex|uint>] [--limit <count>] [--view-mode <Isometric|TopDown>] [--visible-only] [--out <path>]"
        );
        Console.WriteLine();
        Console.WriteLine("Examples:");
        Console.WriteLine("  RenderBufferDump C:\\Games\\Arcanum --map Arcanum1-024-fixed");
        Console.WriteLine(
            "  RenderBufferDump C:\\Games\\Arcanum --kind ObjectAuxiliary --asset-contains light --limit 64"
        );
        Console.WriteLine();
