...
{
    public async Task RunAsync(string saveDir, string[] args)
    {
        ...
        foreach (var (path, bytes) in ctx.Save.Files.OrderBy(entry => entry.Key))
            await WriteLineAsync($"    {bytes.Length, 8:N0}B  {path}");
        ...
        foreach (var (mdyPath, mdyFile) in ctx.Save.MobileMdys.OrderBy(entry => entry.Key))
        {
            ...
            await WriteLineAsync($"    {mdyPath}  records={mdyFile.Records.Count}  v2chars={v2Count}  mobs={mobCount}");
        }
        ...
        if (ctx.AllPcs.Count > 0)
        {
            ...
            await DiagnosticsFieldAnalyzer.WriteLinkFieldSummaryAsync(ctx);
        }
        ...
        foreach (var (typeVal, count) in typeCounts.OrderBy(entry => entry.Key))
        {
            ...
            await WriteLineAsync($"    type={typeVal} ({typeName}): {count}");
        }
    ...
    private static Task WriteLineAsync(string value) => Console.Out.WriteLineAsync(value);
