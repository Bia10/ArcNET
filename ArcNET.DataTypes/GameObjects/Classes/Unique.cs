...
public class Unique : Entity
{
    private static List<int> GetWhitespaceIndexes(string input)
    {
        ...
        foreach ((char item, int index) in stringAsChars.WithIndex())
    ...
    private static Tuple<BasicStatType, int> GetBasicStat(string paramValue)
    {
        ...
        {
            List<int> whitespaceIndexes = GetWhitespaceIndexes(trimmedStats);
        ...
        string[] statAndValue = trimmedStats.Split(separator, 2);
    ...
    private static Tuple<ResistanceType, int> GetResistTuple(string paramName, string paramValue)
    {
        ...
        //ConsoleExtensions.Log($"paramName:|{paramName}| paramValue:|{paramValue}|", "warn");
    ...
    public static Unique GetFromText(IEnumerable<string> uniqueText)
    {
        ...
        foreach (string curLine in uniqueText)
        {
            ...
            try
            {
                switch (paramName)
                {
                    ...
                    {
                        string[] idAndName = paramValue.Split("//", 2);
                        string monsterId = idAndName[0];
                        string monsterName = idAndName[1];
