...
public static class HighResConfig
{
    ...
    {
        foreach ((string key, string value) in ParseIni(iniPath))
            switch (key)
            {
                case "Width":
    ...
    {
        DisplaySettings displaySettings = envInfo.DisplaySettings;
        OperatingSystem osInfo = envInfo.OperatingSystem;
        ...
        if (osInfo.Version.Major is >= 7 and <= 10 || BitDepth == 16)
            DxWrapper = 1;
        else
            DxWrapper = 0;
    ...
    private static IEnumerable<KeyValuePair<string, string>> ParseIni(string iniPath)
    {
        ...
        try
        {
            ...
            while (strLine != null)
            {
                if (strLine != "" && strLine[..2] != "//")
                {
                    string[] keyValuePair = strLine.Split(new[] { '=' }, 2);
    ...
    public static void Write(string iniPath)
    {
        ...
        Type structType = typeof(HighResConfig);
        FieldInfo[] fields = structType.GetFields(BindingFlags.NonPublic | BindingFlags.Static);
